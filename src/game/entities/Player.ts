...
export class Player {
  ...
  constructor(scene: THREE.Scene, x: number, y: number) {
    ...
    this.mesh.position.set(this.position.x, this.position.y, 0.1); // Put player slightly in front of terrain
    scene.add(this.mesh);
    ...
    this.debugColliders.position.set(this.position.x, this.position.y, 0.2);
  ...
  private detectCollisions(world: World) {
    ...
    this.detectHorizontalCollisions(world, bounds);
    ...
    const onGround = this.detectVerticalCollisions(world, boundsAfterHorizontal);
  ...
  private detectHorizontalCollisions(world: World, bounds: any) {
    ...
    for (let i = 0; i < rayCount; i++) {
      ...
      if (this.debugMode) {
        ...
        positions.setXYZ(0, startX, rayOriginY, 0.3);
        positions.setXYZ(1, endX, rayOriginY, 0.3);
  ...
  private detectVerticalCollisions(world: World, bounds: any): boolean {
    ...
    if (this.velocity.y > 0) {
      ...
      for (let i = 0; i < rayCount; i++) {
        ...
        if (this.debugMode) {
          ...
          positions.setXYZ(0, rayOriginX, startY, 0.3);
          positions.setXYZ(1, rayOriginX, endY, 0.3);
          positions.needsUpdate = true;
        }
