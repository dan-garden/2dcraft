...
export function generateChunk(
  ...
  const chunkData: number[][] = Array(chunkHeight).fill(0).map(() => Array(chunkWidth).fill(0));
  
  // Function to set a block in the chunk
  const setBlock = (worldX: number, worldY: number, blockId: number) => {
  ...
  for (let localX = 0; localX < chunkWidth; localX++) {
    ...
    for (let localY = 0; localY < chunkHeight; localY++) {
      ...
      if (blockId === 13) oreCount.coal++;
      else if (blockId === 14) oreCount.iron++;
      else if (blockId === 15) oreCount.gold++;
      else if (blockId === 16) oreCount.diamond++;
      else if (blockId === 17) oreCount.emerald++;
    ...
    try {
      ...
      if (localStructureY >= 0 && localStructureY < chunkHeight) {
        ...
        const isSurfaceBlock = localStructureY + 1 < chunkHeight && 
                            chunkData[localStructureY][localX] !== 0 && 
                            (localStructureY + 1 >= chunkHeight || chunkData[localStructureY + 1][localX] === 0);
